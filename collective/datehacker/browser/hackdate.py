...
import logging
logger = logging.getLogger('collective.datehacker')
...
class HackDateView(BrowserView):
    ...
    def __call__(self, *args, **kw):
        if self.request.form and self.request.form.get('Save'):
            ...
            err.append(self._validate(
                self.request.form.get('modification_date'))
            )
            stateOk = True
            for e in err:
                if e:
            ...
            if stateOk:
                ...
                # I don't use setters method because they are
                # cmf.ManagePortal protected
                if self.request.get('creation_date'):
                    self.context.creation_date = self.request.get('creation_date')
                    # context.getField(creation_field).set(
                    #     context,
                ...
                if self.request.get('modification_date'):
                    self.context.setModificationDate(
                        self.request.get('modification_date')
                    )
                    # context.getField(modification_field).set(
                    #     context,
                    #     context._datify(self.request.get('modification_date'))
                    # )
                    context.reindexObject(
                        idxs=['modified', 'ModificationDate']
                    )
                putils.addPortalMessage(_(u'Dates changed'))
            self.request.response.redirect(
                context.absolute_url() + '/@@hackdates'
            )
            return
    ...
    def _validate(self, datestr):
        ...
        if invalid:
            return _('date_format_invalid',
                     default=u"The string ${date} is not in the right format. \
                               Please fill dates field "
                             u"as 'aaaa/mm/dd hh:mm:ss' or simply \
                             'aaaa/mm/dd'.",
                     mapping={'date': datestr})
        try:
            d = (int(d[:4]), int(d[5:7]), int(d[8:10]), int(
                d[11:13]), int(d[14:16]), int(d[17:19]))
            datetime(*d)
        except ValueError:
